...
#include "vm/vm.h"
#include "threads/vaddr.h"			//! Memory Mapped Pages 추가
#include "userprog/process.h"		//! Memory Mapped Pages 추가
...
void
vm_file_init (void) {			// file-backed page의 하위 시스템을 초기화
}

/* Initialize the file backed page */
bool
file_backed_initializer (struct page *page, enum vm_type type, void *kva) {				// file-backed page를 초기화 함
	/* Set up the handler */
	...
	struct for_lazy *for_lazy = (struct for_lazy *)page->uninit.aux;					//! Memory Mapped Files에서 lazy_load_arg -> for_lazy로 변경
...
file_backed_swap_in (struct page *page, void *kva) {
	...
	// struct for_lazy *for_lazy;				//! 최종 수정시 주석처리
...
file_backed_destroy (struct page *page) {
	// struct file_page *file_page UNUSED = &page->file;
	// todo: 관련된 파일을 닫음으로써 file-backed pages를 파괴한다.
	// todo: 내용이 변경되었다면(dirty) 변경 사항을 파일에 기록해야 한다.
	...
	// page struct를 해제할 필요가 없다. (file_backed_destroy의 호출자가 해야 함)
	// struct file_page *file_page UNUSED = &page->file;
}
...
do_mmap (void *addr, size_t length, int writable,
	...
	while (read_bytes > 0 || zero_bytes > 0)
	{
		...
		if (!vm_alloc_page_with_initializer(VM_FILE, addr,
											writable, lazy_load_segment, for_lazy))
			return NULL;
